---
title: "🏈LinesShines · 鋒光"
weight: 6
toc: true
sidebar:
  hide: true
---


<a href="https://lines-shines.up.railway.app" target="_blank" rel="noopener noreferrer"
   style="display: inline-flex; align-items: center; gap: 10px; height: 52px; padding: 0 1.75rem; box-sizing: border-box;
   background: linear-gradient(135deg, #5c2f0a, #a0522d);
   color: #fff; border-radius: 999px; text-decoration: none; font-weight: 700; font-size: 17px;
   box-shadow: 0 4px 14px rgba(92,47,10,0.5), 0 0 0 1px rgba(255,215,120,0.3) inset;
   transition: transform 0.15s ease, box-shadow 0.15s ease;"
   onmouseover="this.style.transform='translateY(-2px) scale(1.03)'; this.style.boxShadow='0 8px 22px rgba(92,47,10,0.65), 0 0 0 1px rgba(255,215,120,0.4) inset'"
   onmouseout="this.style.transform='none'; this.style.boxShadow='0 4px 14px rgba(92,47,10,0.5), 0 0 0 1px rgba(255,215,120,0.3) inset'">
  <span>🏈</span>
  <span>NFL O & D Lines' Deserved Appreciations</span>
  <span>🙌️</span>
  <span>↗</span>
</a>

| **Backend**                                                                          | **Frontend**     | **Deployment**     |
|--------------------------------------------------------------------------------------|----------------------------|-----------------|
| <img src="https://skillicons.dev/icons?i=python,postgresql,fastapi,github" /> | <img src="https://skillicons.dev/icons?i=js,html,css" /> |  **Railway🛤️** |


## 📈 Platform Characteristics

#### 📇 Player Cards Behind Scatters ☝

<div style="display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; margin: 2rem 0;">
  <img src="platform_home.png" alt="Platform Home"
       style="width: 46%; min-width: 240px; border-radius: 10px; cursor: pointer; transition: transform .15s ease, box-shadow .15s ease;
              border: 3px solid rgba(160,82,45,0.6); box-shadow: 0 0 0 4px rgba(0,0,0,0.3), 0 4px 14px rgba(0,0,0,0.4);"
       onclick="document.getElementById('lb-img').src=this.src; document.getElementById('lightbox').style.display='flex';"
       onmouseover="this.style.transform='scale(1.02)'; this.style.boxShadow='0 0 0 4px rgba(0,0,0,0.3), 0 6px 20px rgba(160,82,45,0.4)'"
       onmouseout="this.style.transform='scale(1)'; this.style.boxShadow='0 0 0 4px rgba(0,0,0,0.3), 0 4px 14px rgba(0,0,0,0.4)'">
  <img src="clickable_player_card.png" alt="Clickable Player Card"
       style="width: 46%; min-width: 240px; border-radius: 10px; cursor: pointer; transition: transform .15s ease, box-shadow .15s ease;
              border: 3px solid rgba(160,82,45,0.6); box-shadow: 0 0 0 4px rgba(0,0,0,0.3), 0 4px 14px rgba(0,0,0,0.4);"
       onclick="document.getElementById('lb-img').src=this.src; document.getElementById('lightbox').style.display='flex';"
       onmouseover="this.style.transform='scale(1.02)'; this.style.boxShadow='0 0 0 4px rgba(0,0,0,0.3), 0 6px 20px rgba(160,82,45,0.4)'"
       onmouseout="this.style.transform='scale(1)'; this.style.boxShadow='0 0 0 4px rgba(0,0,0,0.3), 0 4px 14px rgba(0,0,0,0.4)'">
</div>

LinesShines reveals how shiny an excellent OL/DL is.

__Hereby any plotted franchise logo is on click to show you player's card for detailed stats.__

#### ⇓ Save As You Wish 🤓

<div style="display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; margin: 2rem 0;">
  <img src="save_plot_button.png" alt="Save Plot Button"
       style="width: 46%; min-width: 240px; border-radius: 10px; cursor: pointer; transition: transform .15s ease, box-shadow .15s ease;
              border: 3px solid rgba(160,82,45,0.6); box-shadow: 0 0 0 4px rgba(0,0,0,0.3), 0 4px 14px rgba(0,0,0,0.4);"
       onclick="document.getElementById('lb-img').src=this.src; document.getElementById('lightbox').style.display='flex';"
       onmouseover="this.style.transform='scale(1.02)'; this.style.boxShadow='0 0 0 4px rgba(0,0,0,0.3), 0 6px 20px rgba(160,82,45,0.4)'"
       onmouseout="this.style.transform='scale(1)'; this.style.boxShadow='0 0 0 4px rgba(0,0,0,0.3), 0 4px 14px rgba(0,0,0,0.4)'">
  <img src="saved_plot.png" alt="Saved Plot"
       style="width: 46%; min-width: 240px; border-radius: 10px; cursor: pointer; transition: transform .15s ease, box-shadow .15s ease;
              border: 3px solid rgba(160,82,45,0.6); box-shadow: 0 0 0 4px rgba(0,0,0,0.3), 0 4px 14px rgba(0,0,0,0.4);"
       onclick="document.getElementById('lb-img').src=this.src; document.getElementById('lightbox').style.display='flex';"
       onmouseover="this.style.transform='scale(1.02)'; this.style.boxShadow='0 0 0 4px rgba(0,0,0,0.3), 0 6px 20px rgba(160,82,45,0.4)'"
       onmouseout="this.style.transform='scale(1)'; this.style.boxShadow='0 0 0 4px rgba(0,0,0,0.3), 0 4px 14px rgba(0,0,0,0.4)'">
</div>

Every filtered plot deserves to be saved for future reference.

Hitting that save plot button downloads __a high resolution PNG__ for you.

__Yes, this includes all very clear annotation texts.__


## 📈 HIT: Heroes In Trenches
Perhaps you wonder why I have the mood to hit this platform:

- Fantasy Football causes me to explore __NFL players not mentioned in Fantasy rosters__.

- Being a code builder brings the mindset of **how essential infrastructures are**.

**Their inevitable intersection is, of course, offensive & defensive lines: so-called trenches.**


## ✈️ Steps Forward
In Chinese, highlights are called **"風光 (feng guang)"**, and lines are called **"鋒 (feng)"**.

With "風" and "鋒" sharing **the exact same pronunciation**,
replace "風" with "鋒" gives "鋒光", **meaning highlights of lines.**

In English, __lines & shines rhyme__, so why not 😎

I think these are why I created LinesShines on top of
[my past local NFL plots era](https://github.com/StarsExpress/LinesShines/tree/v0.0-beginning).

<div style="display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; margin: 2rem 0;">
  <img src="local_plots_era.png" alt="Local Plots Era"
       style="width: 46%; min-width: 240px; border-radius: 10px; cursor: pointer; transition: transform .15s ease, box-shadow .15s ease;
              border: 3px solid rgba(160,82,45,0.6); box-shadow: 0 0 0 4px rgba(0,0,0,0.3), 0 4px 14px rgba(0,0,0,0.4);"
       onclick="document.getElementById('lb-img').src=this.src; document.getElementById('lightbox').style.display='flex';"
       onmouseover="this.style.transform='scale(1.02)'; this.style.boxShadow='0 0 0 4px rgba(0,0,0,0.3), 0 6px 20px rgba(6,182,212,0.45)'"
       onmouseout="this.style.transform='scale(1)'; this.style.boxShadow='0 0 0 4px rgba(0,0,0,0.3), 0 4px 14px rgba(0,0,0,0.4)'">
</div>

<div id="lightbox" onclick="this.style.display='none'"
     style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
            background: rgba(0,0,0,0.85); z-index: 9999; align-items: center; justify-content: center; cursor: zoom-out;">
  <img id="lb-img" src="" style="max-width: 90%; max-height: 90%; border-radius: 8px; box-shadow: 0 8px 30px rgba(0,0,0,0.5);">
</div>
